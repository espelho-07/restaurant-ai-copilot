...
import {
  ...
  BarChart3, Activity, TrendingUp, MessageSquare,
  ShoppingCart, ArrowRight, Settings2, MapPin, Building2,
} from "lucide-react";
import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { fetchRecentCalls, computeAnalytics, type RecentCall } from "@/lib/telephonyService";
...
const StatusBadge = ({ status }: { status: string }) => {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    awaiting_location: { label: "Location", cls: "bg-amber-500/10 text-amber-600" },
    awaiting_restaurant_selection: { label: "Select Restaurant", cls: "bg-amber-500/10 text-amber-600" },
    collecting_order: { label: "Taking Order", cls: "bg-amber-500/10 text-amber-600" },
    awaiting_address: { label: "Address", cls: "bg-amber-500/10 text-amber-600" },
    awaiting_confirmation: { label: "Confirmation", cls: "bg-amber-500/10 text-amber-600" },
  };
              ...
              <div className="glass-card p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  ...
                  <div>
                    ...
                    <div className="mt-1 rounded-lg bg-[#0d1117] px-3 py-2 font-mono text-emerald-400 text-[11px]">POST</div>
                  </div>
                  <div>
                    ...
                    <div className="mt-1 space-y-1 font-mono text-[10px] text-muted-foreground">
                      <div><code className="bg-secondary px-1.5 py-0.5 rounded">SUPABASE_URL</code></div>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          ...
          {loading ? (
            ...
            <div className="glass-card p-12 text-center">
              ...
              <button onClick={() => setShowSetup(true)} className="mt-4 rounded-xl bg-emerald-500 px-5 py-2.5 text-xs font-semibold text-white hover:bg-emerald-600 transition-all">
            ...
            <div className="space-y-3">
              {calls.map((call) => (
                <motion.div key={call.callSid} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} layout className="glass-card overflow-hidden">
                  <button onClick={() => setExpandedCall(expandedCall === call.callSid ? null : call.callSid)} className="w-full text-left px-5 py-4 hover:bg-secondary/30 transition-all">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-semibold truncate">{call.orderId ? `Order ${call.orderId}` : `Call ${call.callSid.slice(0, 12)}...`}</span>
                        ...
                        <div className="mt-1 flex flex-wrap gap-2 text-[10px] text-muted-foreground">
                          {call.restaurantName && <span className="inline-flex items-center gap-1"><Building2 className="h-3 w-3" />{call.restaurantName}</span>}
                          {call.selectedCity && <span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" />{call.selectedCity}</span>}
                          {call.orderNumber ? <span>Order #{call.orderNumber}</span> : null}
                        </div>
                      </div>

                      <div className="text-right">
                        {call.total > 0 && <p className="text-sm font-bold">Rs.{call.total}</p>}
                        {(call.foodTotal || call.deliveryCharge) ? (
                          <p className="text-[9px] text-muted-foreground">Food Rs.{call.foodTotal || 0} + Del Rs.{call.deliveryCharge || 0}</p>
                        ) : (
                          <p className="text-[9px] text-muted-foreground">{call.transcript?.length || 0} messages</p>
                        )}
                      </div>
                    </div>
                    ...
                    <p className="mt-2 text-[10px] text-muted-foreground/70 line-clamp-1">
                      {call.deliveryAddress
                        ? `Delivery: ${call.deliveryAddress}${call.pincode ? ` (${call.pincode})` : ""}`
                      ...
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-t border-border/30">
                        ...
                        {call.orderId && (
                          <div className="px-5 py-3 border-t border-border/20">
                            <button onClick={() => navigate("/orders")} className="flex items-center gap-1.5 text-[10px] font-semibold text-emerald-600 hover:text-emerald-700 transition-all">
